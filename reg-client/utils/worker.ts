...
export const useWorkerChannel = ({ onMessage, url }) => {
  ...
  // Django channel groups to filter out from broadcast
  ...
  const onMessageToJson = useMemo(() => {
    ...
    return (message) => {
      const {group, event} = message.data;
      if (groups.has(group)) {
  ...
  useEffect(() => {
    const subscribe = message => {
      ...
      if (data.type === 'websocket-groups' && data.url === url) {
        setGroups(new Set(data.groups));
        port?.removeEventListener('message', subscribe);
      }
    };
    port?.addEventListener('message', subscribe);
    sendData({
  ...
  const readyState = ready && groups.size ? ReadyState.OPEN : ReadyState.CONNECTING;
  return { readyState, sendJsonMessage };
};

export const createWorker = () => {
  const workerUrl = '/2023/worker.js';
