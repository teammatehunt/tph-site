...
def inject_team(
    *,
    ...
    if require_admin or require_internal:
        assert (
            redirect_if_missing
        ), "redirect_if_missing should be True if we require admin or internal accounts"
...
def inject_puzzle(*, allow_admin=True, error_if_inaccessible=True):
    ...
    If the request does not contain a `puzzle` kwarg with the puzzle slug, or if
    the `puzzle` kwarg can not be resolved to a puzzle slug, then a generic error
    is shown.
    ...
    def decorator(view_func):
        ...
        def wrapped(request, *args, **kwargs):
            ...
            try:
                puzzle_slug = kwargs["puzzle"]
                request.puzzle = Puzzle.objects.select_related("round").get(
                    slug=puzzle_slug
                )
                del kwargs["puzzle"]
            except Puzzle.DoesNotExist:
                logger.warning("Cannot find puzzle %s", puzzle_slug)
                return get_inaccessible_puzzle_response(puzzle_slug)

            request.puzzle_access = None
            if team:
                request.puzzle_access = PuzzleAccess.objects.filter(
                    team=team, puzzle=request.puzzle
                ).first()

            if not request.puzzle_access:
                if error_if_inaccessible and not (
                    allow_admin and request.user.is_staff
                ):
                    logger.warning(
                        "Team %s does not have access to puzzle %s",
                        team,
                        request.puzzle,
                    )
                    return get_inaccessible_puzzle_response(puzzle_slug)
...
def inject_round(*, allow_admin=True, error_if_inaccessible=True):
    ...
    If the request does not contain a `round` kwarg with the round slug, or if
    the `round` kwarg can not be resolved to a round slug, then a generic error
    is shown.
    ...
    def decorator(view_func):
        ...
        def wrapped(request, *args, **kwargs):
            ...
            try:
                round_slug = kwargs["round"]
                request.round = Round.objects.get(slug=round_slug)
                del kwargs["round"]
            except Round.DoesNotExist:
                logger.warning("Cannot find round %s", round_slug)
                return get_inaccessible_round_response(round_slug)

            request.round_access = None
            if team:
                request.round_access = RoundAccess.objects.filter(
                    team=team, round=request.round
                ).first()
...
def inject_interaction(*, allow_admin=True, error_if_inaccessible=True):
    ...
    If the request does not contain a `interaction` kwarg with the interaction slug, or if
    the `interaction` kwarg can not be resolved to a interaction slug, then a generic error
    is shown.
    ...
    def decorator(view_func):
        ...
        def wrapped(request, *args, **kwargs):
            ...
            try:
                interaction_slug = kwargs["interaction"]
                request.interaction = Interaction.objects.get(slug=interaction_slug)
                del kwargs["interaction"]
            except Interaction.DoesNotExist:
                logger.warning("Cannot find interaction %s", interaction_slug)
                return get_inaccessible_interaction_response(interaction_slug)

            request.interaction_access = None
            if team:
                request.interaction_access = InteractionAccess.objects.filter(
                    team=team, interaction=request.interaction
                ).first()
    ...
    def wrapped(request, *args, **kwargs):
        ...
        if next_url and not (
            ...
            or url_has_allowed_host_and_scheme(
                next_url, settings.ALLOWED_HOSTS, request.is_secure()
            )
        ):
...
def get_inaccessible_puzzle_response(puzzle_slug):
    return HttpResponseBadRequest("Cannot find puzzle " + escape(puzzle_slug))


def get_inaccessible_round_response(round_slug):
    return HttpResponseBadRequest("Cannot find round " + escape(round_slug))
