...
EVENTS_CACHE_BUCKET = "events"
EVENTS_REF = "events"
A3_EVENTS_CACHE_BUCKET = "a3_events"
A3_EVENTS_REF = "a3_events"
...
def dispatch(event_type, *, message, object_id=None, team=None, **kwargs):
    ...
    if event_type != HuntEvent.HUNT_TICK:
        logger.info(
            'event type=%s message="%s" team=%s object=%s',
            event_type.value,
            message,
            team,
            object_id,
        ...
        SystemLog.objects.create(
            event_type=event_type, message=message, team=team, object_id=object_id
        )

    _dispatch_internal(event_type, message=message, team=team, **kwargs)


def _dispatch_internal(event_type, **kwargs):
    for subscriber, unused_priority in map(
        _resolve_subscriber, subscriptions[event_type]
    ):
        subscriber(**kwargs)
...
@memoized_cache(EVENTS_CACHE_BUCKET)
def get_num_extra_event_rewards(site_ref=EVENTS_REF):
    # import locally to avoid importing before django is set up
    from spoilr.core.models import HuntSetting

    setting, _ = HuntSetting.objects.get_or_create(
        name=f"spoilr.{site_ref}.extra_event_rewards"
    )
    return int(setting.text_value or 0)


@memoized_cache(A3_EVENTS_CACHE_BUCKET)
def get_num_extra_a3_event_rewards(site_ref=A3_EVENTS_REF):
    # import locally to avoid importing before django is set up
    from spoilr.core.models import HuntSetting

    setting, _ = HuntSetting.objects.get_or_create(
        name=f"spoilr.{site_ref}.extra_a3_event_rewards"
    )
    return int(setting.text_value or 0)
