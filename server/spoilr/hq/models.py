...
class Handler(models.Model):
    ...
    def status(self) -> str:
        status = "off duty"
        if self.activity_time and (
            now() - self.activity_time > datetime.timedelta(minutes=5)
        ):
            status = "on duty, idle"
    ...
    @property
    def full_str(self):
        return self.__str__() + f" [{self.status}]"


class HqLog(models.Model):
    timestamp = models.DateTimeField(auto_now_add=True)
    handler = models.ForeignKey(
        Handler, blank=True, null=True, on_delete=models.SET_NULL
    )
...
class Task(models.Model):
    ...
    def __str__(self):
        return f"`{self.content_object}`" + (
            f" -- `{self.handler.discord}`" if self.handler else ""
        )
    ...
    class Meta:
        ...
        constraints = [
            ...
            models.CheckConstraint(
                ...
                check=(
                    (
                        ~models.Q(status=TaskStatus.SNOOZED)
                        & models.Q(snooze_time__isnull=True, snooze_until__isnull=True)
                    )
                    | models.Q(
                        status=TaskStatus.SNOOZED,
                        snooze_time__isnull=False,
