...
def dashboard_view(request):
    contact_requests = (
        ContactRequest.objects.filter(tasks__isnull=False)
        .select_related("team")
    ...
    if request.GET.get("open") == "1":
        ...
        contact_requests = contact_requests.filter(
            resolved_time__isnull=True, tasks__status=TaskStatus.PENDING
        )
...
def respond_view(request):
    ...
    if confirm.lower() != "respond":
        return redirect_with_message(
            request, "spoilr.contact:dashboard", "Contact response was not confirmed."
        )

    contact_request_id = int(request.POST.get("id"))
    contact_request = (
        ContactRequest.objects.select_related("team").get(id=contact_request_id)
    ...
    if not task or task.handler != request.handler:
        return redirect_with_message(
            request,
            "spoilr.contact:dashboard",
            f"You are no longer handling this contact request.",
        )
    if task.status == TaskStatus.DONE:
        return redirect_with_message(
            request, "spoilr.contact:dashboard", "Contact request is already done."
        )
    ...
    HqLog.objects.create(
        ...
        event_type="contact-request-resolved",
        object_id=contact_request.team.username,
        message=f"Resolve contact request {contact_request}",
    )

    dispatch(
        HuntEvent.CONTACT_REQUEST_RESOLVED,
    ...
    if settings.SPOILR_HQ_DEFAULT_FROM_EMAIL:
        ...
        send_mail(
            subject,
            full_response,
            settings.SPOILR_HINTS_FROM_EMAIL,
        ...
        Email.objects.create(
            ...
            from_address=settings.SPOILR_HQ_DEFAULT_FROM_EMAIL,
            to_addresses=contact_request.email,
            team=contact_request.team,
            automated=True,
            handler=request.handler,
        )
    ...
    return redirect_with_message(
        request, "spoilr.contact:dashboard", "Contact request resolved."
    )
